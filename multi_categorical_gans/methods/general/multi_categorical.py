...
class MultiCategorical(nn.Module):
    ...
    def __init__(self, input_size, variable_sizes):
        ...
        self.output_layers.append(nn.Linear(input_size, variable_sizes[0]))
        self.output_activations.append(nn.Sigmoid())

        for i, variable_size in enumerate(variable_sizes[1:]):
    ...
    def forward(self, inputs, training=True, temperature=None, concat=True):
        ...
        logits = self.output_layers[0](inputs)
        output = self.output_activations[0](logits)
        outputs.append(output)
        for output_layer, output_activation in zip(self.output_layers[1:], self.output_activations[1:]):
